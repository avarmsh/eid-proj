...
public class TaskService {
    public static void setAsCompleted(int taskId) {
        try{
            ...
            System.out.println("Task can not be found or couldn't update.");
        }
    }
    ...
    public static void delete(int id){
        try{
            ...
            Database.delete(id);
            System.out.println("Entity ID=" + id + " successfully deleted.");
    ...
    public static void update(int id, String field, String newValue, Boolean isTitle, Boolean isDate){
        try {
            ...
            if(!(entity instanceof Task)){
                System.out.println("Entity ID " + id + " is not a Task.");
                return;
            }
    ...
    public static void updateStatus(int id, int option){
        try{
            ...
            if(option == 1){
                ...
                Database.update(task);
                System.out.println("Status updated to Completed for the Task and all the Steps.");
            } else if(option == 2){
        ...
        try{
            ...
            if(totalSteps > 0){
                ...
                if(completedCount == totalSteps){
                    task.setStatus(Task.Status.Completed);
                    Database.update(task);
                    System.out.println("All steps completed. Task status set to Completed.");
                } else if(task.getStatus() == Task.Status.Completed){
                    task.setStatus(Task.Status.InProgress);
                    Database.update(task);
                    System.out.println("Not all steps completed. Task status set to In Progress.");
                }
            }
